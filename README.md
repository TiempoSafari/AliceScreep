...
> ESJZone 登录抓取建议安装 **Selenium**（会自动优先使用）：`pip install selenium`。
...
ESJZone 若遇到“未发现章节”通常是未登录状态导致，请在 ESJZone 配置中开启登录并填写账号密码（登录页：`https://www.esjzone.cc/my/login`）。程序会优先用 Selenium 浏览器会话登录并抓取；若 Selenium 启动超时（默认 20 秒）或不可用，则自动回退 HTTP 登录。
...
已支持保留网页中的 **Ruby 注音**（如 `<ruby>女<rt>玲子</rt></ruby>`），导出的 EPUB 会尽量按“上方注音”方式显示，而不是把注音直接拼接到正文后。
