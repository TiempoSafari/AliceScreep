...
ESJZone 若遇到“未发现章节”通常是未登录状态导致，请在 ESJZone 配置中开启登录并填写账号密码（登录页：`https://www.esjzone.cc/my/login`）。当前 ESJ 仅使用 Selenium 抓取，并默认以可见浏览器模式启动；若 Selenium 不可用，会直接中止并提示错误。
